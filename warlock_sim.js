...
const TOEP_CD = 90000;
const SHADOW_BOLT_CHANNEL = 2500;
const SHADOW_BOLT_COST = 370;
const SHADOW_BOLT_BASE = 455;
const SHADOW_BOLT_RANGE = 52;
const GCD = 1500;
...
const getSBDamage = (bossStats, bonusSP = 0) => {
  if (Math.random() * 100 < Math.min(BASE_HIT + HIT, 99)) {
    ...
    if (bossStats.isbStacks > 0) {
      modifier += 0.20;
      bossStats.isbStacks -= 1;
    }

    const baseDamage = SHADOW_BOLT_BASE + Math.floor(Math.random() * SHADOW_BOLT_RANGE);
    const spBonus = Math.round((SP + bonusSP) * 0.8571);
    let damage = baseDamage + spBonus

    if (Math.random() * 100 <= CRIT) {
      bossStats.isbStacks = 4;
      bossStats.isbTimer = ISB_DURATION;
      damage *= 2;
    }
...
const runPlayer = (playerStats, bossStats) => {
  ...
  if (playerStats.castTimer == 0 && playerStats.gcdTimer == 0) {
    ...
    if (CS && playerStats.corruptionTimer == 0) {
      ...
      // Handle shadow bolt
      let cast = false;
      if (playerStats.mana >= SHADOW_BOLT_COST) {
        cast = true
      } else {
      ...
      if (cast) {
        playerStats.mana -= SHADOW_BOLT_COST;
        if (playerStats.shadowTrance) {
          ...
          playerStats.castTimer = SHADOW_BOLT_CHANNEL;
